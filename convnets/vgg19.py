...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Conv2D(filters=128, kernel_size=(3, 3), strides=1, padding='same', activation='relu'), # 5
    tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=2),
    ...
    tf.keras.layers.Conv2D(filters=256, kernel_size=(3, 3), strides=1, padding='same', activation='relu'), # 8
    tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=2),
    ...
    tf.keras.layers.Conv2D(filters=512, kernel_size=(3, 3), strides=1, padding='same', activation='relu'), # 14
    tf.keras.layers.Conv2D(filters=512, kernel_size=(3, 3), strides=1, padding='same', activation='relu'), # 15
    tf.keras.layers.Conv2D(filters=512, kernel_size=(3, 3), strides=1, padding='same', activation='relu'), # 16
    tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=2),
    
    tf.keras.layers.Flatten(),

    # Fully connected layer
    tf.keras.layers.Dense(units=4096, activation='relu'), # 17
    tf.keras.layers.Dropout(0.5),
    tf.keras.layers.Dense(units=4096, activation='relu'), # 18
    tf.keras.layers.Dropout(0.5),

    # Units in last layer are 3 per rps dataset
    tf.keras.layers.Dense(units=3, activation='softmax') # 19
], name='VGG19')

if __name__ == '__main__':
    model.summary()
    tf.keras.utils.plot_model(model, os.path.join('architectures', 'vgg19_model.png'), show_shapes=True)
    keras2ascii(model)
    visualkeras.layered_view(model, legend=True, to_file=os.path.join('architectures', 'vgg19_layers.png')).show()
